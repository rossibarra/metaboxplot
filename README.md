...
The example data is regenerated by `example_data/make_example_data.sh` (edit the
`SRC` paths to point at your own source files).
...
interval (crossovers, mutations, peaks). Overlapping event intervals should be
pre-flattened into a non-overlapping bedGraph first (see the crossover binning in
